...
> **Statut — scaffold ; PAS une dépendance Tier-0 (issue #25).** Ces
> fichiers *scaffoldent* le service ; le déploiement live sur le Pi (voir
> Bootstrap) est l'étape suivante de l'opérateur. Une fois déployé il est
> utilisable, mais le homelab n'a pas encore de backup unifié, hors-site
> et testé en restauration (`/mnt/backup` non monté — issue #19). Tant que
> les quatre critères de « graduation » de [BACKUP.md](BACKUP.md) ne sont
> pas verts, garde ton gestionnaire de mots de passe actuel comme source
> de vérité pour tes secrets les plus critiques, et appuie-toi sur la copie
> break-glass décrite là-bas. Voir l'ADR
> [0005](../../docs/decisions/0005-vaultwarden-deployment.md).
...
```bash
# a. Ouvrir brièvement les inscriptions, ré-appliquer l'env (sans perte de
#    données). Mettre VW_SIGNUPS_ALLOWED=true dans .env, puis :
docker compose up -d
#    Garder VW_SIGNUPS_DOMAINS_WHITELIST VIDE : une whitelist non vide
#    laisse ces domaines s'inscrire même quand SIGNUPS_ALLOWED=false (elle
#    l'outrepasse), laissant la porte ouverte après l'étape c.
...
# c. Refermer les inscriptions : remettre VW_SIGNUPS_ALLOWED=false dans
#    .env (et s'assurer que VW_SIGNUPS_DOMAINS_WHITELIST est vide), puis :
docker compose up -d
...
| `VW_SIGNUPS_DOMAINS_WHITELIST` | **Laisser vide.** Une valeur non vide laisse ces domaines s'inscrire même quand `VW_SIGNUPS_ALLOWED=false` (elle l'outrepasse). |
...
- **Instance fermée.** `SIGNUPS_ALLOWED=false` et
  ...
  pas de surface d'invitation. La whitelist de domaines reste vide (elle
  outrepasserait `SIGNUPS_ALLOWED=false`).
